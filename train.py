...
writer = SummaryWriter('relation-loss/loss_11_6')
...
def train(training_dbs, validation_db, system_config, model, args):
    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1), file=save_stdout, ncols=5):
            training = pinned_training_queue.get(block=True)
            training_loss, focal_loss, pull_loss, push_loss, off_loss = nnet.train(**training)
            ...
            if display and iteration % display == 0:
                ...
                print(pull_loss.item())
                print(push_loss.item())
                print(off_loss.item())
                writer.add_scalar('training_loss', training_loss, global_step=iteration)
                writer.add_scalar('train_focal_loss', focal_loss, global_step=iteration)
                writer.add_scalar('train_pull_loss', pull_loss, global_step=iteration)
                writer.add_scalar('train_push_loss', push_loss, global_step=iteration)
                writer.add_scalar('train_off_loss', off_loss, global_step=iteration)
            del training_loss

            if val_iter and validation_db.db_inds.size and iteration % val_iter == 0:
                nnet.eval_mode()
                validation = pinned_validation_queue.get(block=True)
                validation_loss, focal_loss, pull_loss, push_loss, off_loss = nnet.validate(**validation)
                ...
                writer.add_scalar('val_focal_loss', focal_loss, global_step=iteration)
                writer.add_scalar('val_pull_loss', pull_loss, global_step=iteration)
                writer.add_scalar('val_push_loss', push_loss, global_step=iteration)
                writer.add_scalar('val_off_loss', off_loss, global_step=iteration)
                nnet.train_mode()
