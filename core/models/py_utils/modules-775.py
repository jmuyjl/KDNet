...
class hg_net(nn.Module):
    ...
    def _train(self, xs, ys):
        ...
        for i in range(len(br_modules)):
            tl_modules_tl.append(self.relation(tl_heats[i], gt_tl_heats, tl_modules[i].detach()))
            br_modules_br.append(self.relation(br_heats[i], gt_br_heats, br_modules[i].detach()))
    ...
    def _test(self, *xs, **kwargs):
        ...
        tl_mod_tl = self.test_relation(tl_heat, tl_mod)
        br_mod_br = self.test_relation(tl_heat, br_mod)
    ...
    def relation(self, pred_heatmap, gt_heatmap, feature_map):
        ...
        for batch_index, tl_heat in enumerate(pred_heatmap):
            ...
            if gt_kp_position_cat.size(0) != 0:
                ...
                empty_fm_512 = F.relu(self.cnv1(empty_fm_512.unsqueeze(0)))
                feature_map[batch_index] = empty_fm_512.squeeze(0)
                ################################
