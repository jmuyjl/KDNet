...
class hg_net(nn.Module):
    ...
    def relation(self, pred_heatmap, gt_heatmap, feature_map):
        ...
        for batch_index, tl_heat in enumerate(pred_heatmap):
            kp_vector_cat = torch.rand([0]).cuda()
            gt_kp_vector_cat = torch.rand([0]).cuda()
            fg_cat = torch.zeros([0], dtype=torch.int32).cuda()
            gt_fg_cat = torch.zeros([0], dtype=torch.int32).cuda()
            kp_position_cat = torch.zeros([0], dtype=torch.int64).cuda()
            gt_kp_position_cat = torch.zeros([0], dtype=torch.int64).cuda()
            each_1024_new_cat = torch.rand([0]).cuda()
            
            # start_time = time.clock()
            for channel_index, tl_heat_one_channel in enumerate(tl_heat):
                gt_kp_0 = gt_heatmap[batch_index][channel_index] == 1.0
                gt_kp_0 = gt_kp_0.cuda()
                gt_kp_position = torch.nonzero(gt_kp_0)
                gt_kp_position_cat = torch.cat((gt_kp_position_cat, gt_kp_position), 0)
                del gt_kp_0
                kps_num_each_channel[batch_index][channel_index] = gt_kp_position.size(0)
                if gt_kp_position.size(0) == 0:
                    # print("batch_index: " + str(batch_index) + "、channel_index: " + str(
                    #     channel_index) + " is no keypoints（GT Say）???")
                    continue
                else:
            ...
            # print(80*"?")
            # print(kp_position_cat.size())
            if gt_kp_position_cat is not None:
