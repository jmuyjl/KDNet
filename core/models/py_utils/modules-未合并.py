...
class hg_net(nn.Module):
    def __init__(
            ...
            region=5, num_kps=50, fc_dim=1024, cnv_dim=256, out_dim=5
    ):
    ...
    def _train(self, xs, ys):
        ...
        cnvs = self.hg(image)
        # print(80*"1")
        # print("cnvs最小值:{}\tcnvs最大值:{}\tcnvs平均值:{}".format(cnvs[-1].min().item(), cnvs[-1].max().item(), cnvs[-1].mean().item()))
        ...
        # print("tl_modules最小值:{}\ttl_modules最大值:{}\ttl_modules平均值:{}".format(tl_modules[-1].min().item(), tl_modules[-1].max().item(), tl_modules[-1].mean().item()))
        # print("tl_heats最小值:{}\ttl_heats最大值:{}\ttl_heats平均值:{}".format(tl_heats[-1].min().item(), tl_heats[-1].max().item(), tl_heats[-1].mean().item()))
        
        ####################  relation added by yjl 2019/10/26   ####################
        # 这是准的
        # visualize(image, tl_heats[-1], br_heats[-1])
        ...
        for i in range(len(br_modules)):
            tl_modules_tl.append(self.relation(tl_heats[i], gt_tl_heats, tl_modules[i].detach(), image))
            br_modules_br.append(self.relation(br_heats[i], gt_br_heats, br_modules[i].detach(), image))
        
        # print("tl_modules更新后最小值:{}\ttl_modules更新后最大值:{}\ttl_modules更新后平均值:{}".format(tl_modules_tl[-1].min().item(), tl_modules_tl[-1].max().item(), tl_modules_tl[-1].mean().item()))
        ...
        # print("tl_heats更新后最小值:{}\ttl_heats更新后最大值:{}\ttl_heats更新后平均值:{}".format(tl_heats[-1].min().item(), tl_heats[-1].max().item(), tl_heats[-1].mean().item()))
        # print("tl_tags更新后最小值:{}\ttl_tags更新后最大值:{}\ttl_tags更新后平均值:{}".format(tl_tags[-1].min().item(), tl_tags[-1].max().item(), tl_tags[-1].mean().item()))
        # print("tl_offs更新后最小值:{}\ttl_offs更新后最大值:{}\ttl_offs更新后平均值:{}".format(tl_offs[-1].min().item(), tl_offs[-1].max().item(), tl_offs[-1].mean().item()))
        # print(80*"2")
        ####################  relation added by yjl 2019/10/26   ####################
    ...
    def _test(self, *xs, **kwargs):
        ...
        # tl_heats_nms = self._nms(tl_heat, 3)
        # br_heats_nms = self._nms(br_heat, 3)

        tl_mod_tl = self.test_relation(tl_heat, tl_mod, image)
        br_mod_br = self.test_relation(br_heat, br_mod, image)
    ...
    def relation(self, pred_heatmap, gt_heatmap, feature_map, image):
        '''
        ...
        for batch_index, tl_heat in enumerate(pred_heatmap):
            ...
            for channel_index, tl_heat_one_channel in enumerate(tl_heat):
                ...
                gt_kp_position_cat = torch.cat((gt_kp_position_cat, gt_kp_position), 0)
                    # gt_list.append(torch.tensor(index).unsqueeze(0))
                del gt_kp_0
                # if gt_kp_position.size(0) == 0:
                #     continue
                # else:
                    # np.savetxt("/home/jhsu/yjl/lite/test.txt", tl_heat_one_channel.detach().cpu().numpy(), fmt='%1.4f')
                    # print(80*"OK")
            ...
            # print(gt_kp_position_cat)        
            ##################################
            print(80*"?")
            print(gt_kp_position_cat.size(0))
            # draw_proposals(kp_position_cat, image[batch_index], extract=3)
            ##################################
            if gt_kp_position_cat.size(0) != 0:
                ...
                # print(fa_new.size())
                # print(80*"1")
                # print("原始最小值:{}\t原始最大值:{}\t原始平均值:{}".format(feature_map[batch_index].min().item(), feature_map[batch_index].max().item(), feature_map[batch_index].mean().item()))
                ...
                # feature_map[batch_index] = empty_fm.clone()
                ...
                # print("更新最小值:{}\t更新最大值:{}\t更新平均值:{}".format(empty_fm.min().item(), empty_fm.max().item(), empty_fm.mean().item()))
                # print("融合最小值:{}\t融合最大值:{}\t融合平均值:{}".format(feature_map[batch_index].min().item(), feature_map[batch_index].max().item(), feature_map[batch_index].mean().item()))
                # print(80*"2")
                ################################
    ...
    def test_relation(self, pred_heatmap, feature_map, image):
        feature_list = []
        for batch_index, tl_heat in enumerate(pred_heatmap):
            ...
            # draw_proposals(kp_position_cat, image, extract=5)
